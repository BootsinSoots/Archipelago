...
from gclib.dol import DOLSection
...
def update_dol_offsets(lm_gen: "LuigisMansionRandomizer", start_inv: list[str],
    ...
    lm_gen.dol.read(dol_data)

    # Walk Speed
    speed_to_use = 16784 if walk_speed == 0 else (16850 if walk_speed == 1 else 16950)
    lm_gen.dol.data.seek(0x396538)
    lm_gen.dol.data.write(struct.pack(">H", speed_to_use))
    ...
    lm_gen.dol.data.seek(0x7EA28)
    lm_gen.dol.data.write(bytes.fromhex(vac_speed))

    # Fix Boos to properly spawn
    lm_gen.dol.data.seek(0x12DCC9)
    boo_data = "000005"
    lm_gen.dol.data.write(bytes.fromhex(boo_data))
    ...
    if boo_rand_on:
        ...
        lm_gen.dol.data.seek(0x04DB50)
        lm_gen.dol.data.write(bytes.fromhex("93C1FFF0"))

        # Custom boo display counters hooks.
        lm_gen.dol.data.seek(0x04DBB0)
        lm_gen.dol.data.write(bytes.fromhex("4848D469"))
        lm_gen.dol.data.seek(0x04DC10)
        lm_gen.dol.data.write(bytes.fromhex("4848D409"))
    ...
    lm_gen.dol.data.seek(0xCD39B)
    lm_gen.dol.data.write(bytes.fromhex(keys_and_others_val))

    # Diamonds and other treasure animations
    lm_gen.dol.data.seek(0xCE8D3)
    lm_gen.dol.data.write(bytes.fromhex(gem_val))

    # Disable Mario Item pickup animations
    lm_gen.dol.data.seek(0x0CD707)
    lm_gen.dol.data.write(bytes.fromhex(hat_val))

    # Disable Elemental Medal pickup animations
    lm_gen.dol.data.seek(0x0CF4A3)
    lm_gen.dol.data.write(bytes.fromhex(elem_val))
    ...
    if fear_anim_enabled:
        ...
        scare_val = [0x00]
    lm_gen.dol.data.seek(0x396578)
    lm_gen.dol.data.write(struct.pack(">B", *scare_val))

    # Store Player name
    lm_player_name = str(slot_name).strip()
    lm_gen.dol.data.seek(0x324740)
    lm_gen.dol.data.write(sbf.string_to_bytes_with_limit(lm_player_name, LM_PLAYER_NAME_BYTE_LENGTH))

    # Change King Boo's Health
    lm_gen.dol.data.seek(0x399228)
    lm_gen.dol.data.write(struct.pack(">H", king_boo_health))
    ...
    lm_gen.dol.sections[2] = new_dol_sect

    # Append the extra bytes we expect, to ensure we can write to them in memory.
    lm_gen.dol.data.seek(len(lm_gen.dol.data.getvalue()))
    blank_data = b"\x00" * new_dol_size
    lm_gen.dol.data.write(blank_data)
    ...
    lm_gen.dol.data.seek(CUSTOM_CODE_OFFSET_START)
    lm_gen.dol.data.write(custom_dol_code)
    ...
    for csv_line in dol_csv_offsets:
        ...
        lm_gen.dol.data.seek(int(dol_addr, 16))
        lm_gen.dol.data.write(bytes.fromhex(dol_val))

    if not random_spawn == "Foyer":
        spawn_info: LMRegionInfo = REGION_LIST[random_spawn]
        lm_gen.dol.data.seek(0x3A05E0)
        lm_gen.dol.data.write(struct.pack(">f", spawn_info.pos_x))
        lm_gen.dol.data.seek(0x3A05E4)
        lm_gen.dol.data.write(struct.pack(">f", spawn_info.pos_y))
        lm_gen.dol.data.seek(0x3A05E8)
        lm_gen.dol.data.write(struct.pack(">f", spawn_info.pos_z))
    ...
    if dool_model_rando_on:
        ...
        for map_two_doors in door_model_offsets:
            lm_gen.dol.data.seek(map_two_doors)
            door_model_id = random.randint(1,14)
            lm_gen.dol.data.write(door_model_id.to_bytes())

    # Save all changes to the DOL itself.
    lm_gen.dol.save_changes()
    lm_gen.gcm.changed_files["sys/main.dol"] = lm_gen.dol.data